...
namespace Ams.Core
{
    ...
    public class NewKey
    {
        ...
        public static int newmaxplus(IDbContext db, string ID_Name)
        {
            using (SqlConnection conn = new SqlConnection(db.Data.ConnectionString))
            {
                string sp_getnextid = @"yypb.dbo.YY_getsysid";
                string param_id_name = @"sysid_name";
                string param_idnext = @"sysid";
                string param_idFirst = @"first_value";
                if (conn.State != ConnectionState.Open) conn.Open();
                SqlCommand sqlcmd = new SqlCommand(sp_getnextid, conn);
                sqlcmd.CommandType = CommandType.StoredProcedure;
                SqlParameter sql_param = sqlcmd.Parameters.AddWithValue(param_id_name, ID_Name);
                sql_param = sqlcmd.Parameters.AddWithValue(param_idFirst, 1);
                sql_param = sqlcmd.Parameters.Add(param_idnext, SqlDbType.Int);
                sql_param.Direction = System.Data.ParameterDirection.Output;
                sqlcmd.ExecuteNonQuery();
                return Int32.Parse(sql_param.Value.ToString());
            }
        }
        ...
        private static Int64 maxOfAllKey(string cachedKeys, params string[] otherKey)
        {
            ...
            var max = keys.Max<object>(x => Ams.Utils.ZConvert.To<Int64>(x));
            return max;
        }
    }
}
