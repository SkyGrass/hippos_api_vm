...
namespace Ams.Core
{
    public partial class RequestWrapper
    {
        ...
        private void parseWhere(XElement settings, Action<string, string, Func<WhereData, string>, string, object[]> AndWhere, Action<string, string, Func<WhereData, string>, string, object[]> OrWhere = null)
        {
            ...
            /*Extension by YY 如果过滤条件是OR关系的话，用这个*/
            var isAnd = string.Equals(string.IsNullOrEmpty(getXmlElementAttr(where, "isAnd")) ? "true" : getXmlElementAttr(where, "isAnd"),
            ...
            foreach (var item in where.Elements("field"))
            {
                ...
                if (!string.IsNullOrEmpty(value) || !ignoreEmpty)
                {
                    ...
                    {
                        OrWhere(getFieldName(name, true), value, cp, variable, extend);
                    }
                }
        ...
        public ParamQuery ToParamQuery()
        {
            ...
            }, (name, value, compare, variable, extend) =>
            {
                pQuery.OrWhere(name, value, compare, extend);
            });
            return pQuery;
        }
